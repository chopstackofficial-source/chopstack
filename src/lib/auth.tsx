...
export type BuyerProfile = { id: string; name: string; email: string; phone: string | null; delivery_address: string | null; latitude: number | null; longitude: number | null };
export type VendorProfile = { id: string; name: string; email: string; phone: string; status: string; photo_url: string | null; latitude: number | null; longitude: number | null; address: string | null };
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const load = useCallback(async (uid: string) => {
    const [rolesRes, buyerRes, vendorRes] = await Promise.all([
      ...
      supabase.from("buyers").select("id,name,email,phone,delivery_address,latitude,longitude").eq("id", uid).maybeSingle(),
      supabase.from("vendors").select("id,name,email,phone,status,photo_url,latitude,longitude,address").eq("id", uid).maybeSingle(),
    ]);
